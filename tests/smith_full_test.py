...
def plot_example(testbench, title, scale=50, **kwargs):
    print("Testbench '{}' : {}".format(testbench, title.replace("\n", "")))
...
def savefig(testbench):
    for ext in ExportFormats:
        pp.savefig("{}/sample_{}.{}".format(build_path, testbench.lower().replace(" ", "_"), ext), format=ext)
        ...
        for minor in FT:
            for minor_fancy in FT:
                if minor or not minor_fancy:
                    ...
                    plot_example(tb, f"Major: {major_str} - Minor: {minor_str}")
...
def tb_markers():
    ...
    XEndMarker = np.array([
        [0, 0], [0.5, 0.5], [0.25, 0], [0.5, -0.5], [0, 0],
    ...
    for hackline, startmarker, endmarker, rotate_marker in [
        ...
        SmithAxes.update_scParams(
            instance=ax, plot_marker_start=startmarker,
        ...
        plot_example(
            tb, "HackLines: %s - StartMarker: %s\nEndMarker: %s - Rotate: %s" %
            tuple(map(ptype, [hackline, startmarker, endmarker, rotate_marker])), markersize=10,
        )
...
def tb_interpolation():
    ...
    for interpolation, equipoints in [
        ...
        plot_example(
            tb, "Interpolation: {} — Equipoints: {}".format(
                "False" if interpolation is False else interpolation,
                "False" if equipoints is False else equipoints,
...
def tb_misc():
    ...
    pp.subplot(
        2, 3, 3, projection="smith",
        axes_radius=0.3,
    )
    plot_example(tb, "Axes radius: 0.25")

    pp.subplot(
        2, 3, 4, projection="smith",
        symbol_infinity="Inf",
